  ...
  contains
    FUNCTION transform(r,s,t,Bx,By,Bz,Px,Py,Pz) 
      ...
      transform(3) = -(r*(-By**2*Px+Bx*By*Py+Bx*Bz*Pz-Bz**2*Px)+ &
                       s*(-Bz**2*Py+By*Bz*Pz+Bx*By*Px-Bx**2*Py)+ &
                       t*(-Bx**2*Pz+Bx*Bz*Px+By*Bz*Py-By**2*Pz)) / gamma
...
!Parsing script:
  ...
  print *,'# Enter filename (hint: name it MF.txt and place it in the same dir): '
  read *,file
...
!User input:
  ...
  open(unit=11, file='PTMF_3D_'// file_no // 'GeV.dat')
  !file='PTMF_3D_'//CHAR(INT(sqrt(P0x**2+P0y**2+P0z**2)))//'GeV.dat')
  !-------------------------------------------------
